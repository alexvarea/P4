...
for cmd in $*; do #para cada comando en la linia de comandos:
   echo `date`: $cmd '---';
    ...
    if [[ $cmd == train ]]; then
        ...
        for dir in $db_devel/BLOCK*/SES* ; do
            name=${dir/*\/}
            echo $name ----
            EXEC="gmm_train -v 1 -T 0.01 -N 70 -m 60 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train"
            echo $EXEC && $EXEC || exit 1
            echo
        done
    elif [[ $cmd == test ]]; then
       ...
       echo $EXEC && $EXEC | tee $LOG_VERIF || exit 1
        ...
        # realizar este cambio de formato están en el enunciado de la práctica.s
